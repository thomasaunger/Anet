...
class BaseAlgo():
    def __init__(self, env, models, num_frames_per_proc, discount, gae_lambda, preprocess_obss, reshape_reward, use_comm, conventional, argmax, single_precision):
                ...
                with torch.no_grad():
                    for m, model in enumerate(self.models):
                        if torch.any(self.active[:, m]):
                            ...
                            memory[self.active[:, m], m] = model_results["memory"].float() ### casting necessary due to PyTorch Bug?
                            dist                         = model_results["dist"]
                            dists_speaker                = model_results["dists_speaker"]
                            value[ self.active[:, m], m] = model_results["value"].float() ### casting necessary due to PyTorch Bug?
